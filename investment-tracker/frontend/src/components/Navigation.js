  ...
  return (
    <nav className="navigation">
      ...
      <ul className="nav-actions">
        {currentUser && (
          <li>
            <div 
              ...
              title={`${currentUser.name || messages.NAV_EXTRA.USER_FALLBACK}${currentUser.name ? `\n${currentUser.email}` : currentUser.email}${currentUser.provider ? `\n(${currentUser.provider})` : ''}`}
            >
        ...
        <li>
          <button onClick={handleLogout} className="icon-button logout-button" title={messages.NAV_EXTRA.LOGOUT_TOOLTIP}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
